...
create table mark (
    ...
    foreign key (mh_id) references subject_class(mh_id),
...
values 	("Hung", "Ha Noi", "0912113113", 1, 1),
        ...
        select * from student;
insert into subject_class (mh_name, credit, status_mh)
values 	("CF", 5, 1),
		("C", 6, 1),
        ("HDJ", 5, 1),
        ("RDBMS", 10, 1);
...
select * from subject_class where subject_class.credit >= 3 and subject_class.credit <=5;
...
select student.student_name, subject_class.mh_name, mark.mark from student 
inner join subject_class inner join mark 
on (student.student_id = mark.student_id) AND (mark.mh_id = subject_class.mh_id) order by mark desc, student.student_name;